...
<x-app-layout>
    {{-- Styling for the index with each actor being called and put in their own cards. --}}
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[#2b1b1b] border border-yellow-600/30 overflow-hidden shadow-lg sm:rounded-lg">
                <div class="p-6 text-gray-100">
                    <h3 class="font-semibold text-lg mb-4 text-[#f2c94c] border-b border-yellow-600/40 pb-2">List of actors</h3>
                    {{-- The actor Cards along with the grid system they use --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        @foreach($actors as $actor)
                            <div class="bg-[#1f1f1f] border border-yellow-700/40 p-5 rounded-xl shadow-md hover:shadow-yellow-600/20 transition transform hover:-translate-y-1 duration-300">
                                <a href="{{ route('actors.show', $actor) }}"> {{-- links to the show method in the actorController with the correct id --}}
                                    <x-actor-card
                                        :name="$actor->name" {{-- pulls the name from the database with the correct id --}}
                                    />
                                </a>
                                ...
                                <div class="mt-4 flex justify-between">
                                    ...
                                    <a href="{{ route('actors.edit', $actor) }}" 
                                        ...
                                        Edit
                                    </a> {{-- links to the edit method in the actorController with the correct id --}}
                                    <!-- Delete Button -->
                                    <div x-data="{ open: false }"> {{-- Alpine.js state for showing the modal --}}
                                        ...
                                        <div x-show="open" 
                                            ...
                                            <div @click.away="open = false" {{-- closes modal if clicking outside --}}
                                                ...
                                                <p class="mb-6">Are you sure you want to delete this actor? This action cannot be undone.</p>
                                                <div class="flex justify-end space-x-4">
                                                    ...
                                                    <form action="{{ route('actors.destroy', $actor) }}" method="POST">
                                                        ...
                                                        Delete
                                                        </button> {{-- submits to the destroy method in the actorController with the correct id, deletes everything from the database and also from the image folder --}}
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
